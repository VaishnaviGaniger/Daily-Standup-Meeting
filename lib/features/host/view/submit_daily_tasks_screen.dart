...
class SubmitDailyTaskScreens extends StatefulWidget {
  const SubmitDailyTaskScreens({super.key});

  @override
  State<SubmitDailyTaskScreens> createState() => _SubmitDailyTaskScreenState();
}

class _SubmitDailyTaskScreenState extends State<SubmitDailyTaskScreens> {
  final ProjectListController _projectListController = Get.put(
    ProjectListController(),
  );
  final SubmitDailyTasksController _submitDailyTasksController = Get.put(
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Obx(
                      () => _submitDailyTasksController.isLoading.value
                          ? const SizedBox(
                              height: 22,
                              width: 22,
      ...
      child: Column(
        ...
        children: [
          ...
          for (
            ...
            _buildTaskEntry(section, taskIndex),
  ...
  Widget _buildTaskEntry(ProjectSection section, int taskIndex) {
    return Padding(
  ...
  Future<void> _selectDateTime(ProjectSection section, int taskIndex) async {
    ...
    if (pickedTime != null) {
      setState(() {
        section.dateTimes[taskIndex] = DateTime(
  ...
  void submitTasks() async {
    List<SubmitDailyTaskModel> tasks = [];

    for (final section in _projectSections) {
      ...
      for (final c in section.controllers) {
        if (c.text.isEmpty) {
          Get.snackbar(
            'Error',
    ...
    await _submitDailyTasksController.dailyTask(tasks);
